...
def generate_hg_prob_p(num_hypergraphs: int,
    ...
    for i in range(num_hypergraphs):
        ...
        decimal = f"{p:.4f}".split('.')[1]
        main(1, num_nodes, num_hyperedges, p, path, method=method, name=f"{i}_nodes_{num_nodes}_hyperedges_{num_hyperedges}_p_{decimal}_{method}")

def create_diverse_dataset(method: str='from_scratch'):
    list_of_probabilities = [0.005*x for x in range(0, 40)]
    script_dir = os.path.dirname(os.path.abspath(__file__))
    path = os.path.join(script_dir, 'GeneratedHypergraphs')
    os.makedirs(path, exist_ok=True)
    for p in list_of_probabilities:
        # create a folder for each probability
        decimal = f"{p:.4f}".split('.')[1]
        os.makedirs(f'{path}/p_{decimal}', exist_ok=True)
